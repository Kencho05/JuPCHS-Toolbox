...
This property is closely related to Lyapunov stability.  If $S$ has a strict minimum at a certain state $x^\star$, then $x^\star$ is a stable equlibrium point for the unforced dynamics (i.e., $u=0). A storage function for a PCHS can be defined as $S(x)=H(x)-H(x^\star)$ with $x^\star \in \text{argmin} H(x)$. Therefore, the system is passive.
